...
namespace MySql.Parser.Tests
{
  ...
  public class TruncateTable
  {
    ...
    public void Simple()
    {
      AstParserRuleReturnScope<object, IToken> r = Utility.ParseSql("TRUNCATE TABLE table1");
      /*
    ...
    {
      AstParserRuleReturnScope<object, IToken> r = Utility.ParseSql("TRUNCATE `schema1`.`table1`");
      /*
			Assert.Equal(1, statements.Count);
			Assert.True(statements[0] is TruncateTableStatement);
			TruncateTableStatement ts = statements[0] as TruncateTableStatement;
    ...
    {
      Utility.ParseSql("TRUNCATE TABLE ", true);
    }
  }
}
